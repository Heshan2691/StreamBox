...
const MOCK_REVIEWS: Review[] = [
  {
    ...
    rating: 9,
    comment:
      "Fantastic series! The historical accuracy combined with great storytelling makes it a must-watch.",
    date: "2024-01-15",
  },
  {
    id: "r2",
    ...
    rating: 10,
    comment:
      "Masterpiece! Christopher Nolan at his best. Mind-bending and visually stunning.",
    date: "2024-01-20",
  },
];
...
function convertTMDBToMedia(
  ...
  return {
    ...
    releaseYear: parseInt(
      (isMovie ? item.release_date : item.first_air_date)?.split("-")[0] ||
        "2024",
    ),
    ...
    try {
      ...
      try {
        ...
        if (creditsResponse.data.cast) {
          media.cast = creditsResponse.data.cast.slice(0, 10).map(
        ...
        if (
          tvResponse.data.created_by &&
          tvResponse.data.created_by.length > 0
        ) {
    ...
    try {
      ...
      try {
        response = await axios.get(
          `${TMDB_BASE_URL}/movie/${mediaId}/reviews`,
          {
            params: {
              api_key: TMDB_API_KEY,
              language: "en-US",
